...
if df_const is not None:
    ...
    std_upper(df_const, ["COMPANY"])

if df_enf1 is not None:
    df_enf1.columns = [c.strip().upper() for c in df_enf1.columns]
    std_upper(df_enf1, ["NAME OF COMPANY"])
    # Handle missing status/date
    if "STATUS" not in df_enf1.columns:
        df_enf1["STATUS"] = "RESOLVED"
    if "DATE OF RECIEPT" not in df_enf1.columns:
        df_enf1["DATE OF RECIEPT"] = pd.NaT

if df_enf2 is not None:
    df_enf2.columns = [c.strip().upper() for c in df_enf2.columns]
    std_upper(df_enf2, ["NAME OF THE COMPANY"])
    # Fill missing numeric columns with 0 for consistency
    num_cols = ["NO. OF SHAREHOLDERS","RECEIVED","REDRESSED THROUGH EXCHANGE","NON-ACTIONABLE~","ADVISED / OPTED FOR ARBITRATION","PENDING FOR REDRESSAL WITH EXCHANGE"]
    for c in num_cols:
        if c in df_enf2.columns:
            df_enf2[c] = pd.to_numeric(df_enf2[c].replace('-', 0).replace("–", 0), errors="coerce").fillna(0).astype(int)
...
if df_const is not None and (df_enf1 is not None or df_enf2 is not None):
    ...
    for idx, (label, var) in enumerate(metrics):
        with metric_cols[idx]:
            ...
            if choices:
                default = 0
                metric_sources[var] = st.selectbox(label, choices, index=default, key=f"src_{var}")
            else:
    ...
    # -- Total Complaints
    if metric_sources["total_complaints"] == "File 1":
        total_counts = df_enf1.groupby("NAME OF COMPANY").size().reset_index(name="TOTAL_COMPLAINTS_F1")
        merged = merged.merge(total_counts, left_on="COMPANY", right_on="NAME OF COMPANY", how="left")
        merged["TOTAL_COMPLAINTS"] = merged["TOTAL_COMPLAINTS_F1"].fillna(0).astype(int)
    elif metric_sources["total_complaints"] == "File 2":
        if "RECEIVED" in df_enf2.columns:
            merged = merged.merge(df_enf2[["NAME OF THE COMPANY", "RECEIVED"]], 
                                  left_on="COMPANY", right_on="NAME OF THE COMPANY", how="left")
    ...
    # -- Unresolved
    if metric_sources["unresolved"] == "File 1":
        unresolved = df_enf1[df_enf1["STATUS"].str.upper() != "RESOLVED"]
        unresolved_counts = unresolved.groupby("NAME OF COMPANY").size().reset_index(name="UNRESOLVED_F1")
        merged = merged.merge(unresolved_counts, left_on="COMPANY", right_on="NAME OF COMPANY", how="left")
        merged["UNRESOLVED"] = merged["UNRESOLVED_F1"].fillna(0).astype(int)
    else:
        merged["UNRESOLVED"] = 0

    # -- Recent
    if metric_sources["recent"] == "File 1":
        if "DATE OF RECIEPT" in df_enf1.columns:
            today = pd.Timestamp.today()
            one_year_ago = today - pd.Timedelta(days=365)
            df_enf1["DATE_OF_RECIEPT_DT"] = pd.to_datetime(df_enf1["DATE OF RECIEPT"], errors="coerce")
            recent = df_enf1[df_enf1["DATE_OF_RECIEPT_DT"] >= one_year_ago]
            recent_counts = recent.groupby("NAME OF COMPANY").size().reset_index(name="RECENT_F1")
            merged = merged.merge(recent_counts, left_on="COMPANY", right_on="NAME OF COMPANY", how="left")
    ...
    # -- Pending
    if metric_sources["pending"] == "File 2":
        if "PENDING FOR REDRESSAL WITH EXCHANGE" in df_enf2.columns:
            merged = merged.merge(df_enf2[["NAME OF THE COMPANY", "PENDING FOR REDRESSAL WITH EXCHANGE"]],
                                  left_on="COMPANY", right_on="NAME OF THE COMPANY", how="left")
    ...
    # -- Shareholders
    if metric_sources["shareholders"] == "File 2":
        if "NO. OF SHAREHOLDERS" in df_enf2.columns:
            merged = merged.merge(df_enf2[["NAME OF THE COMPANY", "NO. OF SHAREHOLDERS"]],
                                  left_on="COMPANY", right_on="NAME OF THE COMPANY", how="left")
    ...
    merged["ENFORCEMENT_SCORE"] = merged.apply(
        lambda row: safe_eval(
            custom_formula,
            row["TOTAL_COMPLAINTS"], row["UNRESOLVED"], row["RECENT"], row["PENDING"], row["SHAREHOLDERS"],
